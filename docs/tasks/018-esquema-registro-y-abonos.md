...
- Dentro: decidir y documentar si `sales.esPagoCompleto` se elimina o queda como cache (ADR 0013).
  **Resuelto 17-sep: se elimina** (Mani: todo derivado se calcula). Enmienda en el ADR 0013.
- Dentro: actualizar `docs/agents/context.md` si aparece un término nuevo.
- Fuera: las funciones que escriben (002, 019).

## Done cuando
- [x] La migración corre limpio contra una rama de Neon o una base local, no contra producción.
- [x] `npm run typecheck` limpio.
- [x] Un test de esquema verifica los valores del enum contra la tabla del ADR 0015.
