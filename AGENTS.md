...
| Como se califica y se puntua un lead | `calificarEnvio` en `lib/ingesta/calificacion.ts` (T2, T4), con la configuracion de `sources.calificacion`. La calificacion es un HECHO (cuatro reglas del Apps Script, validadas contra 6.400 envios); el puntaje es una ESTIMACION y **sin pesos configurados queda nulo**. Una pregunta configurada que el envio no trae no es "vacio": queda sin calificar y se reporta | `tests/ingesta-calificacion.test.ts` y `tests/ingesta-escritura.test.ts`. Revision manual: **un peso de puntaje escrito en el codigo es el olor** — se ve igual que uno calibrado |
